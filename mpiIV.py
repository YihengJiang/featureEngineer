...
def total_variability(stat_server_file_name,
                      ...
                      tv_init=None,
                      save_init=False,
                      output_file_name=None, logger=None):
    """
    ...
    for it in range(nb_iter):
        if comm.rank == 0:
            logger.info("Start it {}".format(it))
        ...
        for stat_server_file in stat_server_file_name:
            ...
            with h5py.File(stat_server_file, 'r') as fh:
                ...
                comm.Barrier()
                if comm.rank == 0:
                    logger.info("Process file: {}".format(stat_server_file))
        ...
        comm.Barrier()
    if comm.rank == 0:
        logger.info("finish total variability calculate")
    return factor_analyser
...
def EM_split(ubm,
    ...
    for nbg, it in enumerate(iterations[:int(numpy.log2(distrib_nb))]):
        ...
        if comm.rank == 0:
            logger.info("Start training model with {} distributions".format(2 ** nbg))
        ...
        for i in range(it):
            ...
            if comm.rank == 0:
                ...
                # M step
                logger.info("Start M-step, rank {}".format(comm.rank))
            ...
            comm.Barrier()
    if comm.rank == 0:
        ubm.write(output_filename + '_{}.h5'.format(ubm.get_distrib_nb()), prefix='')
    return ubm
