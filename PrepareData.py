...
trial_key = root + "fea/trial_key.h5"
...
class IdMapConstructor():
    @staticmethod
    def keyConstructor(
            reGenerateKey=False):  # if isDefault==True,we will load trial_key from default file,this file is generated by last time.
        if not reGenerateKey:
            return sidekit.Key(root + trial_key)
        else:
            ...
            key.tar = trial
            key.non = nontrial
            key.write(root + trial_key)
            return key

    @staticmethod
    def key2ndx_SelectSomeTrialToDo(key, nontargetNum=False,
                                    reGenerateKey=False):  # nontarget number of each enroll model,if None,it will make all trials.
        if not key:
            key = IdMapConstructor.keyConstructor(reGenerateKey)
        ndx = key.to_ndx()
        if nontargetNum:
...
@ut.timing("TotalTime")
def main():
    IdMapConstructor.keyConstructor(True)
